...
def _log_row(it: int, stats: dict, evals: int, rates: dict, elapsed: float) -> str:
    return (
        ...
        f"PG {rates['pg'] * 100:4.1f}% greedy {rates['greedy']:.0f} | {elapsed:6.1f}s"
    )
...
def main(args: Args | None = None) -> None:
    ...
    print(_log_row(0, stats, evals, rates, time.perf_counter() - t_start), flush=True)
    history.append({"iteration": 0, "evaluations": evals, "buffer": len(trainer.buffer),
                    "elapsed_s": time.perf_counter() - t_start, **stats})
    ...
    for it in range(1, args.iterations + 1):
        ...
        print(_log_row(it, stats, evals, rates, elapsed), flush=True)
